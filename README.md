...
2. このディレクトリでアプリサーバーを起動
   - `node server.js`
   - APIトークンを有効にした場合は `LMSCHAT_API_TOKEN`、`LM_STUDIO_API_TOKEN`、または `LM_API_TOKEN` を設定
3. ブラウザで `http://localhost:5173` を開く
   - LAN内の別端末から使う場合は、起動ログに表示される `LAN   : http://<IPアドレス>:5173` を開く
...
`node server.js` は既定で `0.0.0.0:5173` に bind するため、同じ LAN の端末からアクセスできます。起動ログに表示される `LAN   : http://<IPアドレス>:5173` を別端末のブラウザで開いてください。

bind するアドレスを制限したい場合は `HOST` を指定します。
...
LM Studio 側で API トークンを有効にした場合は、起動前に `LMSCHAT_API_TOKEN`、`LM_STUDIO_API_TOKEN`、または `LM_API_TOKEN` を設定します。
